...
class TestImageEnhancement:
    def test_quality_improvement(self, enhancers, test_image, quality_manager):
        ...
        assert (
            final_metrics["detail_preservation"]
            > initial_metrics["detail_preservation"]
        )
    ...
    def test_recursive_enhancement(self, enhancers, test_image, quality_manager):
        ...
        for i in range(1, len(metrics_history)):
            assert (
                metrics_history[i]["sharpness"] >= metrics_history[i - 1]["sharpness"]
            )
            assert (
                metrics_history[i]["color_accuracy"]
                >= metrics_history[i - 1]["color_accuracy"]
            )
            assert (
                metrics_history[i]["detail_preservation"]
                >= metrics_history[i - 1]["detail_preservation"]
            )
    ...
    def test_enhancement_consistency(self, enhancers, test_image, quality_manager):
        ...
        for i in range(1, len(results)):
            ...
            assert (
                abs(results[i]["color_accuracy"] - results[0]["color_accuracy"]) < 0.1
            )
            assert (
                abs(
                    results[i]["detail_preservation"]
                    - results[0]["detail_preservation"]
                )
                < 0.1
            )
